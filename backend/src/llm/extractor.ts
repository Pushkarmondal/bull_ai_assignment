...
import { rawExtractedReportSchema } from "../validators/report.schema";
...
export async function extractFinancialData(
  ...
  if (!groq) {
    logger.info("Using fallback sample financial extractor (Groq API key not set)");
  ...
  try {
    ...
    const validatedData = rawExtractedReportSchema.parse(jsonParsed);
...
function getSampleExtractedData(companyName: string): RawExtractedReportData {
  return {
    company: {
      name: companyName || "Eternal Ltd.",
    ...
    businessSummary: `${companyName || "Eternal Ltd."}, operates as a market-leading digital platform. Consolidated revenue from operations surged 70.4% YoY in Q1FY26 to Rs. 7,167cr as all business segments demonstrated robust underlying expansion and strong volume growth.`,
    outlook: `${companyName || "Eternal Ltd."} is poised for long-term growth and improved profitability, driven by strong market position. However, significant run-up in price and rich valuations limit upside potential. We maintain a HOLD rating with a target price of Rs. 337.`,
    recommendation: "HOLD",
    highlights: [
      "Consolidated revenue from operations surged 70.4% YoY in Q1FY26 to Rs. 7,167cr as all segments demonstrated robust growth.",
      "Revenue from quick commerce business soared 154.8% YoY to Rs. 2,400cr across core regions.",
      "Net order value (NOV) of B2C businesses rose 55% YoY to Rs. 20,183cr in Q1FY26.",
      "EBITDA margin stood at 1.6% due to strategic expansion and new store additions.",
      "Added 243 new fulfillment stores, bringing total store count to 1,544.",
    ],
    shareholding: [
